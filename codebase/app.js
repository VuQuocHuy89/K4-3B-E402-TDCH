(() => {
  ...
  const pathTopics = [
    { id: pack.topic.id, number: "01", title: "Machine Learning", description: "Nền tảng ML cho lộ trình AI Engineer", available: true },
    { id: "python-data", number: "02", title: "Python & Data", description: "Python, xử lý dữ liệu và công cụ phân tích", available: false },
    { id: "llm-rag", number: "03", title: "LLM & RAG", description: "Language model, retrieval và grounding", available: false },
    { id: "ai-agents", number: "04", title: "AI Agents", description: "Tool use, workflow và agent evaluation", available: false },
    { id: "mlops", number: "05", title: "MLOps & Deployment", description: "Đưa model vào vận hành và theo dõi chất lượng", available: false },
    { id: "computer-vision", number: "06", title: "Computer Vision", description: "Dữ liệu ảnh, vision model và ứng dụng", available: false },
  ];
  ...
  const renderSetup = () => `
    ${pageHeader("PERSONALIZE YOUR PATH", "Bạn muốn học gì tiếp theo?", "Chọn hướng học và tự nhập thời gian của bạn trước khi Pathwise tạo diagnostic và learning roadmap.", "")}
    <div class="setup-layout">
      <section class="setup-main panel-card">
        <div class="setup-step"><span>01</span><div><span class="panel-kicker">MỤC TIÊU</span><h2>Chọn chủ đề muốn bắt đầu</h2><p>Danh sách thể hiện các hướng học của AI Engineer. Bản demo hiện chạy nội dung đầy đủ cho Machine Learning.</p></div></div>
        <div class="topic-choice-grid">
          ${pathTopics.map((topic) => topic.available
            ? `<button class="topic-choice ${state.pathTopic === topic.id ? "is-selected" : ""}" type="button" data-action="select-path-topic" data-topic="${topic.id}" aria-pressed="${state.pathTopic === topic.id}"><span class="topic-choice-icon">${topic.number}</span><span><strong>${escapeHtml(topic.title)}</strong><small>${escapeHtml(topic.description)} · ${escapeHtml(pack.topic.duration)}</small></span>${state.pathTopic === topic.id ? `<span class="selected-check">${icon("check")}</span>` : ""}</button>`
            : `<button class="topic-choice" type="button" disabled aria-disabled="true"><span class="topic-choice-icon">${topic.number}</span><span><strong>${escapeHtml(topic.title)}</strong><small>${escapeHtml(topic.description)}</small></span></button>`).join("")}
        </div>
        <div class="setup-step"><span>02</span><div><span class="panel-kicker">ĐIỂM BẮT ĐẦU</span><h2>Bạn đang ở mức nào?</h2><p>Thông tin này giúp điều chỉnh độ sâu của learning package.</p></div></div>
        <div class="segmented-choice" role="group" aria-label="Trình độ hiện tại">
          ${[["beginner", "Mới bắt đầu", "Cần giải thích từ nền tảng"], ["intermediate", "Đã học một phần", "Biết khái niệm nhưng chưa chắc"], ["advanced", "Đã thực hành", "Muốn đi thẳng vào ứng dụng"]].map(([value, label, help]) => `<button class="level-choice ${state.pathLevel === value ? "is-selected" : ""}" type="button" data-action="select-path-level" data-level="${value}"><strong>${label}</strong><small>${help}</small></button>`).join("")}
        </div>
        ...
        <div class="setup-footer"><span>${!state.pathTopic ? "Chọn một topic để tiếp tục." : !isValidMinutes(state.pathMinutes) ? "Nhập thời gian học để tiếp tục." : "Đã đủ thông tin để tạo learning path."}</span><button class="primary-button" type="button" data-action="create-path" ${state.pathTopic && isValidMinutes(state.pathMinutes) ? "" : "disabled"}>Tạo learning path ${icon("arrow")}</button></div>
      </section>
      <aside class="setup-aside"><div class="setup-preview panel-card"><span class="panel-kicker">SAU KHI THIẾT LẬP</span><h3>Pathwise sẽ làm gì?</h3><div class="setup-preview-row"><span>01</span><p>Đánh giá nền tảng bằng diagnostic</p></div><div class="setup-preview-row"><span>02</span><p>Ưu tiên competency gap</p></div><div class="setup-preview-row"><span>03</span><p>Tạo roadmap và learning package</p></div><div class="setup-preview-row"><span>04</span><p>Kiểm tra mastery để mở bước tiếp</p></div></div><div class="setup-note panel-card"><span class="application-icon">${icon("shield")}</span><div><strong>Không chọn hộ bạn</strong><p>Machine Learning chỉ bắt đầu sau khi bạn xác nhận topic này.</p></div></div></aside>
    </div>`;

  const renderOverview = () => {
    const firstSection = getSection();
    const diagnosticDone = state.diagnosticSubmitted;
    const nextTitle = diagnosticDone ? firstSection.title : "Làm diagnostic đầu vào";
    const nextCopy = diagnosticDone ? "Bắt đầu section được ưu tiên từ kết quả đánh giá của bạn." : `${pack.diagnosticQuestions.length} câu hỏi · khoảng 5 phút · không trừ điểm`;
    ...
    return `
      ${pageHeader("LEARNING PATH · AI ENGINEER", `Chào ${escapeHtml(pack.learner.name)}, bắt đầu đúng nền tảng.`, `Pathwise đọc mục tiêu, diagnostic và tài liệu ${escapeHtml(pack.topic.documentName)} để sắp xếp bước học phù hợp.`, `<button class="secondary-button compact-button" type="button" data-action="topic-menu">${icon("route")} ${escapeHtml(pack.topic.module)} <span class="chevron">⌄</span></button>`)}
      <div class="overview-top-grid">
        <article class="mastery-hero panel-card">
          ...
          <div class="mastery-hero-main"><div class="progress-ring" style="--progress:${topicProgress()}"><div><strong>${topicProgress()}</strong><span>%</span></div></div><div><h2>${pack.topic.title}</h2><p>${pack.topic.objective}</p><div class="inline-meta"><span>${icon("file")} ${pack.competencies.length} competencies</span><span>${icon("clock")} ${pack.topic.duration}</span></div></div></div>
  ...
  const renderDiagnostic = () => {
    ...
    return `
      ${pageHeader("DIAGNOSTIC · ĐÁNH GIÁ ĐẦU VÀO", "Thử trước khi học", `${pack.diagnosticQuestions.length} câu hỏi để xác định nền tảng hiện tại. Confidence signal giúp lộ ra những câu đúng do hiểu hay chỉ đang đoán.`, `<span class="assessment-rule"><span class="rule-icon">${icon("shield")}</span><span><strong>Không trừ điểm</strong><small>Pass mark sẽ áp dụng ở mastery test</small></span></span>`)}
  ...
  const submitDiagnostic = async () => {
    const payload = {
      ...
      answers: pack.diagnosticQuestions.map((question) => ({
    ...
    try {
      ...
      state.aiAnalysis = localAnalysis();
    ...
    state.diagnosticSubmitted = true;
    state.view = "diagnostic-result";
    window.history.replaceState(null, "", "#diagnostic-result");
    render();
    showToast(state.agentMeta?.live ? "Agent đã tạo learning profile có căn cứ." : "Đã tạo learning profile bằng fallback an toàn.");
  };
  ...
  const renderRoadmap = () => {
    ...
    const gapCount = state.aiAnalysis?.competency_gaps?.length || 2;
    return `
    ...
    <div class="roadmap-summary"><div><span class="summary-label">PERSONALISED PLAN</span><h2>Ôn gap trước, học mới sau</h2><p>${state.diagnosticSubmitted ? `Kết quả diagnostic đã được dùng để ưu tiên ${escapeHtml(prioritySection.title)}.` : "Làm diagnostic để tạo lộ trình sát với kiến thức hiện tại."}</p></div><div class="summary-stats"><div><strong>${gapCount}</strong><span>gap ưu tiên</span></div><div><strong>${state.timePlan}</strong><span>phút dự kiến</span></div><div><strong>80%</strong><span>ngưỡng pass</span></div></div></div>
  ...
  const render = () => {
    ...
    if (topicSwitcher) {
      ...
      topicSwitcher.querySelector("strong").textContent = state.profileConfigured ? pack.topic.module : "Thiết lập learning path";
    }
    const mobileContext = document.querySelector(".mobile-context > span");
    if (mobileContext) mobileContext.textContent = state.profileConfigured ? pack.topic.module : "Thiết lập learning path";
  ...
  const reset = () => {
    Object.assign(state, { view: "setup", profileConfigured: false, pathTopic: "", pathLevel: "beginner", pathMinutes: "", diagnosticIndex: 0, diagnosticAnswers: {}, diagnosticConfidence: {}, diagnosticSubmitted: false, aiAnalysis: null, agentMeta: null, recommendedPath: [], selectedSectionId: "section-ml-foundations", timePlan: 30, focusStarted: false, studyCompleted: false, studyChecks: [], masteryIndex: 0, masteryAnswers: {}, masterySubmitted: false, masteryScore: null, masteryPassed: false, completedSections: {}, remediationData: null, remediationText: "", remediationChecked: false, remediationLoading: false, discoveredSources: {}, sourceDiscoveryLoading: false, generatedPackages: {}, packageLoading: false, finalStarted: false, finalIndex: 0, finalAnswers: {}, finalSubmitted: false, finalScore: null, tutorLoading: false, tutorMessages: [{ role: "assistant", text: "Bạn có thể hỏi về problem framing, supervised learning, regression, overfitting hoặc model evaluation. Mình sẽ trả lời dựa trên tài liệu đã được gắn nguồn.", sources: [], confidence: "high" }] });
  ...
  document.addEventListener("click", (event) => {
    ...
    if (action === "topic-menu") { if (!state.profileConfigured) setView("setup"); else showToast(`Đang học ${pack.topic.title} từ ${pack.topic.documentName}.`); }
    ...
    if (action === "select-path-topic") { state.pathTopic = target.dataset.topic; render(); }
    ...
    if (action === "create-path") { if (!state.pathTopic) { showToast("Hãy chọn một topic trước."); return; } if (!isValidMinutes(state.pathMinutes)) { showToast("Hãy nhập thời gian học từ 10 đến 240 phút."); document.querySelector("#path-minutes")?.focus(); return; } state.profileConfigured = true; state.timePlan = Number(state.pathMinutes); state.view = "overview"; window.history.replaceState(null, "", "#overview"); render(); showToast("Đã tạo learning path. Bước tiếp theo là diagnostic."); }
    if (action === "start-diagnostic") { state.diagnosticIndex = 0; setView("diagnostic"); }
    ...
    if (action === "redo-diagnostic") { state.diagnosticIndex = 0; state.diagnosticAnswers = {}; state.diagnosticConfidence = {}; state.diagnosticSubmitted = false; state.aiAnalysis = null; state.agentMeta = null; setView("diagnostic"); }
  ...
  document.addEventListener("input", (event) => {
    ...
    if (event.target.id === "path-minutes") {
      ...
      if (createButton) createButton.disabled = !state.pathTopic || !valid;
      if (footerMessage) footerMessage.textContent = !state.pathTopic ? "Chọn một topic để tiếp tục." : !valid ? "Nhập thời gian học để tiếp tục." : "Đã đủ thông tin để tạo learning path.";
